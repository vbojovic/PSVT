...
//        $(this._displayObj).height($(this._displayObj).parent().height());
        ...
        switch (this._method){
            ...
            case "helicalWheel":
                ...
                break;
            case "star":
                var data = this.drawStar(this._scaleName,35,35);
...
//        console.log(scale);
    ...
    Calc.prototype.drawStar=function(scaleName, xScale, yScale){
        if (this._displayObj == undefined || this._displayObj == null) return;
        if (this._seq == undefined || this._seq == '') return;
        var self = this;
//        var colors = this.get;
        var zoom = function(){
...
//            var color = self.getHydrophobicColor(aa);
    ...
    Calc.prototype.aaCodeConvert=function(aa){
        ...
        if (codes[aa] != undefined) return codes[aa];
...
//    Calc.prototype.record2tableRow=function(data){
//        var keys = Object.keys(data);
//        var html=[];
//        for (var i = 0 ; i < keys.length ; i++){
//            var key = keys[i];
//            if (this._disabledKeys.indexOf(key)>-1) continue;
...
//        return html.join("");
//    };
//    Calc.prototype.printArrayTable=function(data,clearTable){
//        if (clearTable === true) 
//            $(this._resultTableObj).html("");
//        if (data==null){
...
//        for (var i = 0 ; i < keys.length ; i++){
//            var key = keys[i];
//            if (disabledKeys.indexOf(key)>-1) continue;
//            var descr = ( Descriptions[key] === undefined) ? key : Descriptions[key];
//            var val = data[keys[i]];
//            html.push("<tr>");
//                html.push("<td>");
